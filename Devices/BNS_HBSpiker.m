classdef BNS_HBSpiker < handle
    % BNS_HBSPIKER Create a client for communicating with the Backyardbrains
    ...
    %   communicates with a spikerbox over serialport PORT and sets the size of the
    %   input buffer (for continuous mode) to CONTBUFFERDURATION.  When
    ...
    methods (Access = public)
        function obj = BNS_HBSpiker(varargin)
            ...
            if nargin > 1
                ...
                %make sure the duration leads to an integer number of
                %samples
        ...
        function Start(obj)
            ...
            %check for a hand shake from the device
            tic
